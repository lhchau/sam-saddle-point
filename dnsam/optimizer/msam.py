...
class MSAM(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        for i, param in enumerate(params_with_grad):
            ...
            # Back to w_{t} from w_{t+0.5}
            param.sub_(buf, alpha = rho*group["inverse_norm_buffer"])
            
            if buf is None:
                buf = torch.clone(d_p).detach()
                momentum_buffer_list[i] = buf
            else:
                # Update v_{t} = mu * v_{t-1} + g_{t}
                buf.mul_(momentum).add_(d_p, alpha=1-momentum)
            
            # Update w_{t+1} = w_{t} - lr * g_{t}
        ...
        for i, param in enumerate(params_with_grad):
            param.add_(momentum_buffer_list[i], alpha = rho*group["inverse_norm_buffer"])
