import torch
from dnsam.utils import enable_running_stats, disable_running_stats
import contextlib
from torch.distributed import ReduceOp

class GDSAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, model, gdsam_alpha, rho=0.05, adaptive=False, perturb_eps=1e-12, grad_reduce='mean', **kwargs):
        defaults = dict(adaptive=adaptive, **kwargs)
        super(GDSAM, self).__init__(params, defaults)
        self.model = model
        self.base_optimizer = base_optimizer(self.param_groups, **kwargs)
        self.param_groups = self.base_optimizer.param_groups
        self.adaptive = adaptive
        self.perturb_eps = perturb_eps
        self.alpha = gdsam_alpha
        self.rho_t = rho
    ...
    def gradient_decompose(self, alpha=0.0):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                vertical = p.grad.data - cosine * new_grad_norm * self.state[p]['old_g'] / (old_grad_norm + self.perturb_eps)
                p.grad.data = self.state[p]['old_g']
                p.grad.data.add_(vertical, alpha=alpha)
